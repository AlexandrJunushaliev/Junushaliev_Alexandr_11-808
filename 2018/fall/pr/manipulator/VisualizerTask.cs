...
namespace Manipulation
{
    public static class VisualizerTask
    {
        ...
        public static void KeyDown(Form form, KeyEventArgs key)
        {
            ...
            else if (key.KeyCode == Keys.A) Shoulder -= 0.1;
            else if (key.KeyCode == Keys.W) Elbow += 0.1;
            else if (key.KeyCode == Keys.S) Elbow -= 0.1;
        ...
        public static void MouseMove(Form form, MouseEventArgs e)
        {
            ...
            var point = ConvertWindowToMath(new PointF(e.X, e.Y), shoulderPos);
            //получение логических координат 
            X = point.X;
            Y = point.Y;
            UpdateManipulator();
            form.Invalidate();
        ...
        public static void DrawManipulator(Graphics graphics, PointF shoulderPos)
        {
            ...
            DrawManipulatorLine(graphics, shoulderPos, new PointF(0, 0), joints[0]);
            DrawManipulatorLine(graphics, shoulderPos, joints[0], joints[1]);
            DrawManipulatorLine(graphics, shoulderPos, joints[1], joints[2]);

            var radius = 15;
            DrawManipulatorJoint(graphics, shoulderPos, radius, new PointF(0, 0));
            DrawManipulatorJoint(graphics, shoulderPos, radius, joints[0]);
            DrawManipulatorJoint(graphics, shoulderPos, radius, joints[1]);
            DrawManipulatorJoint(graphics, shoulderPos, radius, joints[2]);
        }
        private static void DrawManipulatorLine(Graphics graphics, PointF shoulderPos, PointF point1, PointF point2)
        {
            graphics.DrawLine(
                ManipulatorPen,
                ConvertMathToWindow(point1, shoulderPos),
                ConvertMathToWindow(point2, shoulderPos)
            );
        }
        private static void DrawManipulatorJoint(Graphics graphics, PointF shoulderPos, int radius, PointF joint) {
            var point = ConvertMathToWindow(joint, shoulderPos);
